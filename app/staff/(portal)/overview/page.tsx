    ...
    <main className="mx-auto max-w-7xl space-y-5 px-4 py-5 pb-28 sm:px-6 lg:py-7 lg:pb-7">
      ...
      <section className="overflow-hidden rounded-2xl border border-[var(--line)] bg-white shadow-2xs" aria-labelledby="staff-registration-list">
        <div className="flex flex-col gap-3 border-b border-[var(--line)] p-4 sm:p-5 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="relative w-full lg:max-w-sm"><Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-[var(--muted)]" /><input value={query} onChange={(event) => setQuery(event.target.value)} className="editorial-input pl-10 text-sm" placeholder="ค้นหารหัส หรือชื่อผู้บริจาค" /></div>
        </div>
        <div className="flex gap-2 overflow-x-auto border-b border-[var(--line)] px-4 py-3 sm:px-5">
          {filters.map((item) => <button key={item.value} type="button" onClick={() => setFilter(item.value)} className={`shrink-0 rounded-full px-3 py-1.5 text-xs font-bold ${filter === item.value ? 'bg-[var(--burgundy-700)] text-white' : 'bg-[var(--bg)] text-[var(--muted)] hover:bg-[var(--rose-100)]'}`}>{item.label}</button>)}
        </div>
