...
export default function AdminContentPage() {
  ...
  const handleUpdateBlock = (key: string, field: string, value: any) => {
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-8 sm:px-6">
      ...
      <div className="border-b border-[#FCE8EC] pb-6">
      ...
      {loading ? (
        ...
        <div className="mt-8 space-y-6">
          {blocks.map((block) => (
            <div key={block.id} className="rounded-3xl border border-[#FCE8EC] bg-white p-6 shadow-sm">
              
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-gray-100 pb-4">
                <div>
                  <span className="text-[10px] font-bold tracking-wider text-[#B42336] uppercase">Key: {block.content_key}</span>
                  <h2 className="text-base font-extrabold text-[#29272A]">{block.title}</h2>
                </div>
                ...
                <div className="flex items-center gap-3">
                  <label className="flex items-center gap-2 text-xs font-semibold text-gray-700 cursor-pointer">
                    <input
                      ...
                      className="rounded border-gray-300 text-[#7A1020] focus:ring-[#7A1020]"
                    />
                    แสดงผลสาธารณะ
                  </label>
                  ...
                  <button
                    ...
                    className="inline-flex items-center gap-1.5 rounded-xl bg-[#7A1020] px-4 py-2 text-xs font-bold text-white shadow hover:bg-[#8F1327] disabled:opacity-50"
                  >
              ...
              <div className="mt-4 grid grid-cols-1 gap-4 md:grid-cols-12">
                <div className="md:col-span-8 space-y-3">
                  <div>
                    <label className="block text-[11px] font-bold text-gray-700 mb-1">หัวข้อสื่อ (Title)</label>
                    <input
                      type="text"
                      value={block.title}
                      onChange={(e) => handleUpdateBlock(block.content_key, 'title', e.target.value)}
                      className="w-full rounded-xl border border-gray-300 px-3 py-2 text-xs text-[#29272A] focus:outline-none focus:ring-2 focus:ring-[#7A1020]"
                    />
                  </div>

                  <div>
                    <label className="block text-[11px] font-bold text-gray-700 mb-1">คำอธิบายเพิ่มเติม (Description)</label>
                    <textarea
                      rows={2}
                  ...
                  <div>
                    <label className="block text-[11px] font-bold text-gray-700 mb-1">URL รูปภาพ (Image URL / Storage Public URL)</label>
                    <input
                      type="text"
                      placeholder="https://storage.supabase.co/storage/v1/object/public/media/poster.png"
                  ...
                  <div className="rounded-2xl border border-gray-200 bg-slate-50 p-4 aspect-video flex flex-col items-center justify-center text-center overflow-hidden">
                    {block.image_url ? (
                      ...
                      <>
                        ...
                        <span className="text-[10px] text-gray-400 mt-0.5">ยังไม่ได้อัปโหลดไฟล์ภาพจริง</span>
                      </>
                    )}
                  </div>
                </div>
